...
public class QuestionsXmlImport {
    ...
    private QuestionDetailsDto importMultipleChoiceQuestion(Element questionElement) {
        ...
        for (Element optionElement : questionElement.getChild("options").getChildren("option")) {
            ...
            Integer orderPos = null;

            // if XML attribute exists i.e. if option has order
            if (optionElement.getAttributeValue("orderPos") != null) {
                orderPos = Integer.valueOf(optionElement.getAttributeValue("orderPos"));
            }
    ...
    private QuestionDetailsDto importItemCombinationQuestion(Element questionElement) {
        ...
        for (Element itemElement : questionElement.getChild("items").getChildren("item")) {
            Integer optionSequence = Integer.valueOf(itemElement.getAttributeValue("sequence"));
            String optionContent = itemElement.getAttributeValue("content");

            List<Association> connection = new ArrayList<>();
            for (Element connectionElement : questionElement.getChild("items").getChildren("connections")) {
                for (Element associationElement : connectionElement.getChildren("association")) {
                    Integer id = Integer.valueOf(associationElement.getAttributeValue("id"));
                    String link = String.valueOf(associationElement.getAttributeValue("itemTwo"));
                    connection.add(new Association(id, link));
                }
            }

            ItemDto itemDto = new ItemDto();
            itemDto.setSequence(optionSequence);
            itemDto.setContent(optionContent);
            itemDto.setAssociations(connection);
    ...
    private QuestionDetailsDto importOpenEndedQuestion(Element questionElement) {
        ...
        questionDto.setMaxCharacters(Integer.valueOf(questionElement.getAttributeValue("maxChars")));
        questionDto.setProposedAnswer(questionElement.getAttributeValue("proposedAnswer"));
