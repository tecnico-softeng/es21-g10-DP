describe('Manage Item Combination Questions Walk-through', () => {
    ...
    function validateQuestionFull(
      ...
      validateQuestion(title, content, itemPrefix);
    ...
    it('Creates a new item combination question', function () {
      ...
      cy.get('button').contains('Connect 1 with 2').click();
      cy.get('button').contains('Connect 3 with 2').click();
      cy.get('button').contains('Connect 2 with 1').click();
      cy.get('button').contains('Connect 2 with 3').click();
    ...
    it('Can view an item combination question (with button)', function () {
      cy.get('tbody tr')
        .first()
        .within(($list) => {
    ...
    it('Can view an item combination question (with click)', function () {
      cy.get('[data-cy="questionTitleGrid"]').first().click();
    ...
    it('Can duplicate and item combination question', function () {
      cy.get('tbody tr')
        .first()
        .within(($list) => {
      ...
      cy.get('[data-cy="addItemItemCombination"]').click({ force: true });
      cy.get('[data-cy="Delete5"]').click({ force: true });
      cy.route('POST', '/courses/*/questions/').as('postQuestion');
      cy.get('button').contains('Save').click();
      cy.wait('@postQuestion').its('status').should('eq', 200);
